...
def load_retail_data(data_path: str) -> list:
    ...
    if 'Customer_Segment' in df.columns:
        overview_text += f"- Customer segments: {df['Customer_Segment'].nunique()} segments\n"
    overview_text += f"- Total records: {len(df)}\n- Number of columns: {len(df.columns)}\n"
    
    documents.append({
        "text": overview_text,
        "source": "dataset_overview",
        "metadata": {"type": "overview", "rows": len(df), "columns": len(df.columns)}
    })
    ...
    if 'Product_Category' in df.columns:
        ...
        logger.warning("Column 'Product_Category' not found. Skipping category analysis.")
    ...
    if 'Customer_Segment' in df.columns:
        ...
        logger.warning("Column 'Customer_Segment' not found. Skipping segment analysis.")
    
    # Document 4: Sales trends by month
    if 'Date' in df.columns and 'Sales_Amount' in df.columns:
        try:
            df['Month'] = pd.to_datetime(df['Date']).dt.month
            monthly_sales = df.groupby('Month')['Sales_Amount'].agg(['sum', 'count', 'mean']).round(2)
        ...
        logger.warning("Columns 'Date' or 'Sales_Amount' not found. Skipping monthly sales trends.")
    ...
    if 'Product_Name' in df.columns and 'Sales_Amount' in df.columns:
        ...
        logger.warning("Columns 'Product_Name' or 'Sales_Amount' not found. Skipping top products analysis.")
...
def test_rag_pipeline():
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        try:
            ...
            logger.info(f"Retrieval time: {result.pipeline_metrics['retrieval_time_ms']:.2f}ms")
            logger.info(f"LLM time: {result.pipeline_metrics['llm_generation_time_ms']:.2f}ms")
            logger.info(f"Context length: {result.pipeline_metrics['context_length']} chars")
            logger.info(f"Chunks retrieved: {result.pipeline_metrics['num_chunks_retrieved']}")
            
            # Log retrieval quality
            quality = result.pipeline_metrics['retrieval_quality']
            logger.info(f"Retrieval quality - Avg similarity: {quality['avg_similarity']:.3f}, "
    ...
    if successful_results:
        ...
        avg_retrieval_time = sum(r.pipeline_metrics['retrieval_time_ms'] for r in successful_results) / len(successful_results)
        avg_llm_time = sum(r.pipeline_metrics['llm_generation_time_ms'] for r in successful_results) / len(successful_results)
        
        logger.info(f"Successful queries: {len(successful_results)}/{len(test_queries)}")
        ...
        if avg_total_time < 10000:  # 10 seconds
            ...
            logger.warning("⚠️ Performance target not met: Response times over 10 seconds")
