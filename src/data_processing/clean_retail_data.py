...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"\nData Validation:")
    print(
        f"Customer ID range: {df_clean['customer_id'].min()} to {df_clean['customer_id'].max()}"
    )
    print(f"Age range: {df_clean['age'].min()} to {df_clean['age'].max()}")
    print(
        f"Purchase amount range: ${df_clean['purchase_amount_usd'].min():.2f} to ${df_clean['purchase_amount_usd'].max():.2f}"
    )
