...
class DocumentRetriever:
    ...
    def retrieve(self, query: str, top_k: Optional[int] = None) -> RetrievalResult:
        ...
        logger.info(f"Retrieved {len(chunks)} chunks for query '{query[:50]}...' in {retrieval_time_ms:.2f}ms")
        return result
    
    def format_context(self, retrieval_result: RetrievalResult, max_context_length: int = 2000) -> str:
        """
        ...
        for i, (chunk, score) in enumerate(zip(retrieval_result.retrieved_chunks, retrieval_result.similarity_scores)):
        ...
        logger.info(f"Formatted context with {len(context_parts)} chunks ({len(context)} characters)")
        return context
    ...
    def analyze_retrieval_quality(self, retrieval_result: RetrievalResult) -> Dict[str, Any]:
        """
        ...
        coverage = len(retrieval_result.retrieved_chunks) / retrieval_result.total_chunks_searched
        
        # Diversity (number of unique sources)
        unique_sources = len(set(chunk.source_document for chunk in retrieval_result.retrieved_chunks))
        ...
        logger.info(f"Retrieval quality - Avg similarity: {avg_similarity:.3f}, "
