...
class FactualAccuracyScorer:
    ...
    def calculate_factual_accuracy(self, response: str, ground_truth: str) -> float:
        ...
        try:
            ...
            precision = (
                len(correct_facts) / len(response_facts) if response_facts else 0.0
            )
...
class TokenEfficiencyScorer:
    ...
    def calculate_token_efficiency(
        self, response: str, tokens_used: int, query: str
    ) -> float:
        """
...
class CompletenessScorer:
    ...
    def calculate_completeness(
        ...
        try:
            ...
            # Combine scores
            completeness_score = (
                0.5 * term_coverage + 0.3 * length_score + 0.2 * conclusion_score
            )
...
class ResponseTimeScorer:
    ...
    def calculate_time_efficiency(
        self, response_time_ms: float, response_length: int
    ) -> float:
        """
...
class EvaluationMetricsCalculator:
    ...
    def calculate_all_metrics(
        ...
        try:
            ...
            factual_accuracy = self.factual_scorer.calculate_factual_accuracy(
                response, ground_truth
            )
            token_efficiency = self.token_scorer.calculate_token_efficiency(
                response, tokens_used, query
            )
            coherence = self.coherence_scorer.calculate_coherence(response)
            completeness = self.completeness_scorer.calculate_completeness(
                response, query, context
            )
            time_efficiency = self.time_scorer.calculate_time_efficiency(
                response_time_ms, len(response)
            )
            ...
            confidence_interval = (
                max(0, overall_score - std_dev),
                min(1, overall_score + std_dev),
            )
