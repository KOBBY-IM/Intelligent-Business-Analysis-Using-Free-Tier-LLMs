...
from .metrics import EvaluationMetricsCalculator, EvaluationMetrics
from .ground_truth import GroundTruthManager, GroundTruthAnswer
...
class LLMEvaluator:
    ...
    def evaluate_single_response(self, 
        ...
        try:
            ...
            logger.info(f"Evaluated {provider_name}/{model_name} for question {question_id}")
            return result
    ...
    def run_batch_evaluation(self, 
                           ...
                           difficulties: Optional[List[str]] = None) -> BatchEvaluationResult:
        """
        ...
        try:
            ...
            if question_ids:
                questions = [self.ground_truth_manager.get_answer(qid) for qid in question_ids]
            ...
            logger.info(f"Evaluating {len(questions)} questions across {len(providers_to_test)} providers")
            ...
            for question in questions:
                for provider_name in providers_to_test:
                    ...
                    if not provider.health_check():
                        logger.warning(f"Provider {provider_name} is not healthy, skipping")
                        continue
                    
                    # Get models for this provider
                    provider_models = provider.list_models()
                    if model_names:
                        provider_models = [m for m in provider_models if m in model_names]
                    
                    for model_name in provider_models:
                        try:
                            logger.info(f"Evaluating {provider_name}/{model_name} for question {question.question_id}")
                            ...
                            if llm_response.success:
                                ...
                                result = self.evaluate_single_response(
                                    ...
                                    response_time_ms=llm_response.latency_ms or response_time,
                                    tokens_used=llm_response.tokens_used or 0
                                )
                                results.append(result)
                            else:
                                logger.warning(f"Failed to get response from {provider_name}/{model_name}: {llm_response.error}")
                            
                            completed_evaluations += 1
                            logger.info(f"Progress: {completed_evaluations}/{total_evaluations}")
    ...
    def _generate_summary_stats(self, results: List[EvaluationResult]) -> Dict[str, Any]:
        ...
        try:
            ...
            for provider, provider_results_list in provider_results.items():
                overall_scores = [r.metrics.overall_score for r in provider_results_list]
                relevance_scores = [r.metrics.relevance_score for r in provider_results_list]
                factual_scores = [r.metrics.factual_accuracy for r in provider_results_list]
                response_times = [r.metrics.response_time_ms for r in provider_results_list]
            ...
            summary['overall_stats'] = {
                ...
                'best_provider': max(provider_results.keys(), 
                                   key=lambda p: summary['provider_stats'][p]['avg_overall_score']),
                'worst_provider': min(provider_results.keys(), 
                                    key=lambda p: summary['provider_stats'][p]['avg_overall_score'])
            }
    ...
    def _perform_statistical_analysis(self, results: List[EvaluationResult]) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis = {
                ...
                'effect_sizes': self.stats_analyzer.calculate_effect_sizes(provider_results)
            }
    ...
    def _serialize_pairwise_comparisons(self, comparisons: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Dict[str, Any]]]:
        ...
        for provider1, provider1_comparisons in comparisons.items():
            ...
            for provider2, result in provider1_comparisons.items():
                serialized[provider1][provider2] = self._serialize_statistical_result(result)
        return serialized
    ...
    def _save_evaluation_results(self, batch_result: BatchEvaluationResult):
        ...
        try:
            ...
            results_file = self.results_dir / f"{batch_result.evaluation_id}_results.json"
            ...
            # Save summary report
            summary_file = self.results_dir / f"{batch_result.evaluation_id}_summary.txt"
    ...
    def _generate_text_summary(self, batch_result: BatchEvaluationResult, output_file: Path):
        ...
        try:
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                for provider, data in rankings.items():
                    ...
                    f.write(f"  Mean Score: {data.get('mean_score', 0):.4f}\n")
                    f.write(f"  Std Dev: {data.get('std_score', 0):.4f}\n")
                    f.write(f"  Confidence Interval: {data.get('confidence_interval', (0, 0))}\n\n")
                ...
                if anova_result:
                    ...
                    f.write(f"Effect Size: {anova_result.get('effect_size', 0):.4f}\n\n")
                ...
                f.write(f"Worst Provider: {overall_stats.get('worst_provider', 'N/A')}\n")
                f.write(f"Average Overall Score: {overall_stats.get('avg_overall_score', 0):.4f}\n")
                f.write(f"Average Response Time: {overall_stats.get('avg_response_time_ms', 0):.2f}ms\n\n")
                
        except Exception as e:
            logger.error(f"Error generating text summary: {e}")
    
    def load_evaluation_results(self, evaluation_id: str) -> Optional[BatchEvaluationResult]:
